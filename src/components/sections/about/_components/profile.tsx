...
export const AboutProfile = forwardRef<HTMLElement, AboutProfileProps>(
    ({ src, alt, className }, ref) => {
        return (
            <figure
                ref={ref}
                className={`${className} relative shrink-0 group [perspective:2000px] w-full max-w-[480px] mx-auto lg:mx-0`}
            >
                {/* 1. Elemento de fundo (Sombra de acento flutuante) */}
                ...
                {/* 2. Container da Moldura 3D Principal */}
                <div className="relative w-full aspect-[3/4] bg-white/10 p-[5px] rounded-[2.8rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.10)] transition-all duration-700 ease-out group-hover:[transform:rotateX(6deg)_rotateY(-12deg)_translateZ(40px)] border-b-[6px] border-r-[6px] border-black/10 overflow-hidden group/card shadow-accent/5">

                    {/* 3. Container da Imagem com Borda Interna */}
                    <div className="relative w-full h-full bg-white rounded-[2.7rem] p-3">
