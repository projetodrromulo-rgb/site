...
import { Procedure } from "@/components/sections/procedures/types";
...
export default function ProcedurePageClient({ procedure, footerContent }: ProcedurePageClientProps) {
    ...
    return (
        <div className="min-h-screen bg-[#0A192F] text-white flex flex-col font-sans selection:bg-[#0db9f2]/30">
            ...
            <main className="flex-1 container mx-auto px-6 pt-32 pb-24 md:pt-48 max-w-6xl">
                ...
                {/* FAQ Accordion Section for Procedures */}
                {faqItems.length > 0 && (
                    <div className="mt-16 md:mt-24 pt-12 border-t border-white/10">
                        <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-8 flex items-center gap-2.5 font-display">
                            <HelpCircle className="text-[#0db9f2]" size={32} />
                        ...
                        <div className="space-y-4 max-w-4xl">
                            {faqItems.map((item, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className="border border-white/10 rounded-2xl overflow-hidden shadow-sm bg-white/5 backdrop-blur-md transition-colors"
                                    >
                                        <button
                                            onClick={() => toggleFaq(index)}
                                            className="w-full text-left py-4 px-6 flex items-center justify-between font-bold text-white hover:text-[#0db9f2] transition-colors focus:outline-none bg-white/[0.02]"
                                        >
                                        ...
                                        <div
                                            className={`transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? "max-h-[1000px] border-t border-white/10" : "max-h-0"
                                                }`}
                                        >
                                            <div className="p-6 text-white/70 text-sm md:text-base leading-relaxed bg-white/[0.01]">
                ...
                {/* References Section for Procedures */}
                {procedure.references && procedure.references.length > 0 && (
                    <div className="mt-16 md:mt-24 pt-12 border-t border-white/10">
                        <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-8 flex items-center gap-2.5 font-display">
                            <span className="text-[#0db9f2]">📚</span>
                            Referências
                        </h2>

                        <div className="grid grid-cols-1 gap-6 max-w-4xl">
                            {procedure.references.map((ref: string, idx: number) => (
                                <div
                                    key={idx}
                                    className="bg-white/5 backdrop-blur-md rounded-2xl p-8 border border-white/10 text-white/70 text-sm md:text-base leading-relaxed font-sans text-left"
                                >
                                    {ref}
                                </div>
                            ))}
